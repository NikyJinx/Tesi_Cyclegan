...
# Dilation crescente nei 6 blocchi residui del bottleneck.
# La sequenza 1,2,4,8 amplia il campo recettivo per catturare dipendenze
# tempo/frequenza piu lunghe; il reset finale 1,2 rifinisce i dettagli locali.
dilations = [1, 2, 4, 8, 1, 2]

class ResidualBlock2D_GLU(nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=1):
        super().__init__()
        # padding = dilation * (kernel_size // 2) keeps spatial dims unchanged
        self.conv = nn.Conv2d(channels, channels * 2, kernel_size=kernel_size,
...
class Generator_2_1_2D(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.res_blocks = nn.Sequential(
            *[ResidualBlock2D_GLU(64, dilation=d) for d in dilations]
        )
...
if __name__ == "__main__":
    ...
    with open(log_path, "a", encoding="utf-8") as log_file:
        with contextlib.redirect_stdout(Tee(sys.stdout, log_file)):
            ...
            print("=" * 90)
            train_cyclegan(
                domain_b_dir="E:\\Dataset_Tesi\\dataset_dominio_Violin",
                domain_a_dir="E:\\Dataset_Tesi\\dataset_dominio_Piano",
                checkpoint_dir=checkpoint_dir,
                #d_update_min_loss=0.3,
                #resume_path=os.path.join("checkpoints", "checkpoint_epoch_030.pt"),
            )
