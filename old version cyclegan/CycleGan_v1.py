...
# ==========================================
def train_cyclegan(
    domain_a_dir,       # Cartella .npy del dominio A
    domain_b_dir,       # Cartella .npy del dominio B
    ...
    print(f"Lambda cycle: {lambda_cycle}  |  Lambda identity: {lambda_identity}  |  Lambda trig: {lambda_trig}")
    print("=" * 90)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for batch_idx, (real_A, real_B) in enumerate(zip(loader_A, loader_B), 1):
            ...
            # Totale Generatori
            loss_G = loss_G_adv + loss_G_cycle + loss_idt + loss_trig
            loss_G.backward()
            opt_G.step()
            ...
            # Aggiorna i discriminatori solo se non sono troppo forti
            # Sotto 0.085 non ricevono gradiente, così i generatori possono recuperare
            if loss_D.item() > 0.085:
                loss_D.backward()
                opt_D.step()
...
if __name__ == "__main__":
    train_cyclegan(
        ...
        num_epochs=100,
        batch_size=4,
        resume_path="checkpoints/checkpoint_epoch_020.pt",
    )
