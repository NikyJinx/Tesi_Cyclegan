...
from Cyclegan_V6_modified import Generator_2_1_2D
...
DEFAULT_CHECKPOINT_PATH = "C:\\CycleGan\\STFT\\checkpoints_Lofi_Piano\\checkpoint_epoch_036.pt"
DEFAULT_GENERATOR_KEY = "G_BA"


def extract_epoch_from_checkpoint(checkpoint_path: str) -> str:
    checkpoint_name = os.path.basename(checkpoint_path)
    match = re.search(r"epoch_(\d+)", checkpoint_name, re.IGNORECASE)
    if match:
        return match.group(1)
    return "unknown"


def build_generator_output_dir(base_output_dir: str, generator_key: str, checkpoint_path: str) -> str:
    epoch = extract_epoch_from_checkpoint(checkpoint_path)
    subfolder_name = f"{generator_key}_epoch_{epoch}"
    return os.path.join(base_output_dir, subfolder_name)
...
def load_generator(checkpoint_path: str, generator_key: str, device: torch.device) -> Generator_2_1_2D:
    ...
    try:
        ...
        raise RuntimeError(
            "Checkpoint non compatibile con Cyclegan_V6_modified. "
            "Usa un checkpoint addestrato con la stessa architettura "
            "(kernel asimmetrici e dilatazione temporale)."
        ) from exc
...
def inference_and_reconstruct(
    ...
    generator_key: str = DEFAULT_GENERATOR_KEY,
    batch_size: int = 1,
    ...
    output_audio_dir = build_generator_output_dir(output_audio_dir, generator_key, checkpoint_path)
    ...
    print(f"Cartella output: {output_audio_dir}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Inferenza audio per Cyclegan_V6_modified")
