# MODIFICHE APPLICATE DA CYCLEGAN_V6
# lambda_cycle  =   3.0 -> 1.2
# Forma dei Kernel (Blocchi Residuali)
# 	Prima: Kernel quadrati fissi passati come kernel_size=3.  
# 	Ora: Kernel asimmetrici e rettangolari estratti dalla lista temporal_kernels contenente le coppie (3, 7) e (1, 9).  

# Gestione della Dilatazione (Generatore)
# 	Prima: Dilatazione bidimensionale simmetrica passata come dilation=d. Questa agiva contemporaneamente 
#   sia sull'asse del tempo sia sull'asse delle frequenze.  
# 	Ora: Dilatazione unidimensionale passata come dilation=(1, d). 
#   L'espansione avviene esclusivamente sull'asse orizzontale del tempo.  

# Calcolo del Padding (Classe ResidualBlock2D_GLU)
# 	Prima: Calcolo scalare singolo padding = dilation * (kernel_size // 2).  
# 	Ora: Calcolo dinamico e indipendente per asse padding = (dilation[0] * (kernel_size[0] // 2), dilation[1] * (kernel_size[1] // 2)) 
#   per mantenere invariate le dimensioni con i kernel rettangolari.  

# Controllo dei Tipi di Dato (Classe ResidualBlock2D_GLU)
# 	Prima: Il codice si aspettava implicitamente solo valori interi.  
# 	Ora: Inserimento dei controlli isinstance(kernel_size, int) e isinstance(dilation, int) per convertire 
#   in automatico i valori singoli in tuple e gestire i nuovi formati asimmetrici in totale sicurezza.
...
#Aggiunta di ncreasing dilation rates across the six residual blocks

dilations = [1, 2, 4, 8, 1, 2]  # doubling + reset
temporal_kernels = [(3, 7), (1, 9), (3, 7), (1, 9), (3, 7), (1, 9)]

class ResidualBlock2D_GLU(nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=1):
        ...
        # padding per asse per mantenere HxW invariato anche con kernel rettangolari
        padding = (
    ...
    def forward(self, x):
        ...
        # Gated Linear Unit (rimane invariata)
...
class Generator_2_1_2D(nn.Module):
    def __init__(self, in_channels=3):
        ...
        # Fase 2: Collo di bottiglia 2D (ora mescola TEMPO e FREQUENZE)
        # Suggerimento: visto che in un altro punto volevi ridurre l'input a 256x256, 
        # se lo fai puoi aumentare i blocchi da 6 a 9 per dare più "capacità" alla rete!
        self.res_blocks = nn.Sequential(
            *[
                ResidualBlock2D_GLU(
                    64,
                    kernel_size=k,
                    dilation=(1, d),  # dilatazione solo temporale (asse orizzontale)
                )
                for k, d in zip(temporal_kernels, dilations)
            ]
        )
...
class NpyDataset(Dataset):
    """Carica tensori .npy (3, 512, 2048) da una cartella."""
    def __init__(self, folder, expected_shape=(3, 512, 2048), validate_files=True):
            ...
            for path in files:
                try:
                    ...
                    if arr.shape != self.expected_shape:
                        invalid_files.append((path, f"shape {arr.shape}"))
                        continue
                    valid_files.append(path)
                except Exception as exc:
                    invalid_files.append((path, f"errore load: {exc}"))

            self.files = valid_files

            if invalid_files:
                print(f"[NpyDataset] Scartati {len(invalid_files)} file non validi in '{folder}'.")
                for path, reason in invalid_files[:5]:
                    print(f"    - {path} -> {reason}")
                if len(invalid_files) > 5:
                    print(f"    ... altri {len(invalid_files) - 5} file scartati")
        else:
            self.files = files

        if not self.files:
            raise ValueError(
                f"Nessun file valido in '{folder}'. Attesa shape: {self.expected_shape}."
            )
    ...
    def __getitem__(self, idx):
        path = self.files[idx]
        try:
            tensor = np.load(path, allow_pickle=False).astype(np.float32)
        except Exception as exc:
            raise ValueError(f"Errore caricando '{path}': {exc}") from exc

        if tensor.shape != self.expected_shape:
            raise ValueError(
                f"Shape non valida in '{path}': {tensor.shape}, attesa {self.expected_shape}"
            )
...
def train_cyclegan(
    ...
    print(f"D update every: {d_update_every} batch  |  D min loss to update: {d_update_min_loss}")
    print("=" * 90)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for batch_idx, (real_A, real_B) in enumerate(zip(loader_A, loader_B), 1):
            ...
            should_update_d = (batch_idx % d_update_every == 0)
            if d_update_min_loss is not None:
                should_update_d = should_update_d and (loss_D.item() >= d_update_min_loss)
        ...
        print(f"    D updates : {d_updates}  |  D skips: {d_skips}")
        print("=" * 90)
...
if __name__ == "__main__":
    checkpoint_dir = "checkpoints_Violin_Classic_Jazz"
    os.makedirs(checkpoint_dir, exist_ok=True)
    ...
    with open(log_path, "a", encoding="utf-8") as log_file:
        with contextlib.redirect_stdout(Tee(sys.stdout, log_file)):
            ...
            print("=" * 90)
            train_cyclegan(
                domain_a_dir="E:\\Dataset_Tesi\\dataset_dominio_Lofi",
                domain_b_dir="E:\\Dataset_Tesi\\dataset_dominio_Piano",
                checkpoint_dir=checkpoint_dir,
                #d_update_min_loss=0.3,
                #resume_path=os.path.join("checkpoints", "checkpoint_epoch_027.pt"),
            )
